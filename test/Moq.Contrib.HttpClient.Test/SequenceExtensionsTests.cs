...
namespace Moq.Contrib.HttpClient.Test
{
    public class SequenceExtensionsTests
    {
        ...
        public SequenceExtensionsTests()
        {
            ...
            _client.BaseAddress = new Uri("https://example.com");
        }
        ...
        {
            var url = new Uri(_client.BaseAddress, "value");

            // Simulate a service outage to test retry logic
            _handler.SetupRequestSequence(url)
            ...
            {
                var result = await Policy.HandleResult<HttpResponseMessage>(r => r.StatusCode == HttpStatusCode.ServiceUnavailable)
                                         .RetryAsync(3)
                                         .ExecuteAsync(() => _client.GetAsync("value"));
            ...
            _handler.VerifyRequest(url, Times.Exactly(4));
        }
        ...
        public async Task CanDefineMultipleSetupsToMatchInSequence(bool doItCorrectly)
        {
            ...
            var encodeUrl = new Uri(_client.BaseAddress, "chevrons/encode");
            var lockUrl = new Uri(_client.BaseAddress, "chevrons/lock");
            ...
            for (var i = 1; i <= 6; i++)
            {
                _handler.InSequence(sequence)
                        .SetupRequest(HttpMethod.Post, encodeUrl)
                        .ReturnsResponse($"Chevron {i} encoded!");
            }

            _handler.InSequence(sequence)
                    .SetupRequest(HttpMethod.Post, lockUrl, async r => (await r.Content.ReadAsStringAsync()) == PointOfOrigin)
            ...
            async Task<string> PostAndReadString(Uri uri, string content = "")
            {
                ...
                using (var response = await _client.PostAsync(uri, new StringContent(content)))
                {
